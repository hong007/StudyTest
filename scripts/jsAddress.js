...
var addressInit = function(cmbP,cmbC,cmbA,defaultP,defaultC,defaultA){
    ...
    for(i = 0; i < areaData.length; i++){
        areaStrId = areaData[i].id.toString().substr(2,4);//取id的后四位
        areaStrPid = areaData[i].pid.toString();
        //若为0000则是省或者直辖市
        if(areaStrId == '0000'){
            cmdAddOption(cmbProvince,areaData[i].id,areaData[i].name,areaData[i]);
        }else if(areaData[i].id.toString()=='86'){//否则判断是否为id为86即是否为全国
...
var  cmdAddOption =  function (cmb,id,str,obj){
    if (navigator.userAgent.indexOf("MSIE") > 0) {
        ...
        cmb.insertBefore(option, cmb.options[cmb.options.length]);
    }else{
        var length = cmb.options==null?0:cmb.options.length;
        cmb.insertBefore(new Option(str, id), cmb.options[length]);
    }

}

var changeCity =  function (){
    if('-1' != cmbCity.selectedIndex.toString()){
        optionStrId = cmbCity.options[cmbCity.selectedIndex].value.toString().substr(0,4);
        cmbArea.options.length = 0;
        cmbArea.onchange = null;
        if(cmbCity.selectedIndex == -1)return;
        cmdAddOption(cmbArea,"","-请选择区-","");
        alert("12");
        // for(i = 0; i < areaData.length; i++){
        //     areaStrId = areaData[i].id.toString().substr(0,4);
        //     areaStrIdLst = areaData[i].id.toString().substr(4,2);
        //     if(optionStrId == areaStrId && areaStrIdLst != '00'){
        //         cmdAddOption(cmbArea,areaData[i].id,areaData[i].name,areaData[i]);
        //     }
        // }
...
var changeProvince = function(){
    ...
    optionStrId = cmbProvince.options[cmbProvince.selectedIndex].value.toString().substr(0,2);
    cmbCity.options.length = 0;
    cmbCity.onchange = null;
    if(cmbProvince.selectedIndex == -1)return;
    cmdAddOption(cmbCity,"","城市",'');
    for(i = 0; i < areaData.length; i++){
        ...
        if(optionStrId == areaStrId && areaStrIdLst == '00' && areaStrIdMid != '00'){
            ...
            cmdAddOption(cmbCity,areaData[i].id,areaData[i].name,areaData[i]);
        }
    }
...
var addressInit2 = function(cmbP,cmbC,cmbA,defaultP,defaultC,defaultA){

    cmbProvince2 = document.getElementById(cmbP);
    cmbCity2 = document.getElementById(cmbC);
    cmbArea2 = document.getElementById(cmbA);
    defaultProvince2 = defaultP;
    defaultCity2 = defaultC;
    defaultArea2 = defaultA;
    for(i = 0; i < areaData.length; i++){
        areaStrId = areaData[i].id.toString().substr(2,4);
        areaStrPid = areaData[i].pid.toString();
        if(areaStrId == '0000'){
            cmdAddOption(cmbProvince2,areaData[i].id,areaData[i].name,areaData[i]);
        }else if(areaData[i].id.toString()=='86'){
            cmdAddOption(cmbProvince2,'',areaData[i].name,areaData[i]);
        }
    }
    cmbSelect2(cmbProvince2, defaultProvince2);
    changeProvince2();
    cmbProvince2.onchange = changeProvince2;
    cmbCity2.onchange = changeCity2;
...
//var changeCity2 =  function (){
    //if('-1' != cmbCity2.selectedIndex.toString()){
      //  optionStrId = cmbCity2.options[cmbCity2.selectedIndex].value.toString().substr(0,4);
      //  cmbArea2.options.length = 0;
      //  cmbArea2.onchange = null;
      //  if(cmbCity2.selectedIndex == -1)return;
      //  cmdAddOption2(cmbArea2,"","-请选择区-","");
      //  for(i = 0; i < areaData.length; i++){
         //   areaStrId = areaData[i].id.toString().substr(0,4);
          //  areaStrIdLst = areaData[i].id.toString().substr(4,2);
          //  if(optionStrId == areaStrId && areaStrIdLst != '00'){
           //     cmdAddOption2(cmbArea2,areaData[i].id,areaData[i].name,areaData[i]);
           // }
       // }
      //  cmbSelect2(cmbArea2, defaultArea2);
  //  }else{
    //    cmbArea2.options.length = 0;
     //   cmbArea2.onchange = null;
   // }
//}
var cmbSelect2 = function(cmb, str)
{
    for(var i=0; i<cmb.options.length; i++)
    {
        if(cmb.options[i].value == str)
...
var changeProvince2 = function(){
    //alert(cmbProvince.selectedIndex);
    optionStrId = cmbProvince2.options[cmbProvince2.selectedIndex].value.toString().substr(0,2);
    cmbCity2.options.length = 0;
    cmbCity2.onchange = null;
    if(cmbProvince2.selectedIndex == -1)return;
    cmdAddOption2(cmbCity2,"","-请选择市-",'');
    for(i = 0; i < areaData.length; i++){
        areaStrId = areaData[i].id.toString().substr(0,2);
        areaStrIdMid = areaData[i].id.toString().substr(2,2);
        areaStrIdLst = areaData[i].id.toString().substr(4,2);
        if(optionStrId == areaStrId && areaStrIdLst == '00' && areaStrIdMid != '00'){
            ...
            cmdAddOption2(cmbCity2,areaData[i].id,areaData[i].name,areaData[i]);
        }
    }
    cmbSelect2(cmbCity2, defaultCity2);
    changeCity2();
    cmbCity2.onchange = changeCity2;
}

var areaData=[
    {id:86,name:"全国",pid:"",type:"country",pinyin:"zhong guo",py:"zg",prefix:"z"},
...
var cityData={
    hots:[
        {id:'',name:"",pid:"86",type:"city",pinyin:"",py:"",prefix:""},
        {id:110100,name:"北京市",pid:"110000",type:"city",pinyin:"bei jing shi",py:"bjs",prefix:"b"},
        {id:310100,name:"上海市",pid:"310000",type:"city",pinyin:"shang hai shi",py:"shs",prefix:"s"},
        {id:440100,name:"广州市",pid:"440000",type:"city",pinyin:"guang zhou shi",py:"gzs",prefix:"g"},
        {id:440300,name:"深圳市",pid:"440000",type:"city",pinyin:"shen zhen shi",py:"szs",prefix:"s"},
