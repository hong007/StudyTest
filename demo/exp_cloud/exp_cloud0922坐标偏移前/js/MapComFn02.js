...
function ruleTurn(){
    ...
    if(mapRightMenu.drawRule){
        ...
        if($('#rule_zoom').length > 0){
            ...
            div.append($('<h5 style="line-height: 50px;text-align: center;margin: 0;"></h5>').text('距离为: 0 米'))
    ...
    if(mapRightMenu.drawRightMenu){
        ...
        google.maps.event.addListener(mapRightMenu.map,'rightclick',function (event) {
            ...
            if($('#context-menu').length > 0){
                ...
                // console.log(event.pixel.x,event.pixel.y);
                $(mapRightMenu.target).append(menu.css({'z-index':9999,left:event.pixel.x,top:event.pixel.y}));
            }
