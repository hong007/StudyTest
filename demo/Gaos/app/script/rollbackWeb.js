...
$('.jindu').on('mousedown',function (e) {
    ...
    if(long.length==0){
        ...
        if(0 < param2.countTime && param2.countTime < (long[0].t - param2.startTime)*1000/param2.timestep){
            ...
            flights[long[0].id].info.setPosition({lng:long[long.length-1].lng*1, lat:long[long.length-1].lat*1});
...
function subPlay2() {
    ...
    if(eachPopulateStep<=param2.steps){
        rollbackflight.setPosition({lng:p1.lng*1 + (p2.lng - p1.lng) * eachPopulateStep / param2.steps, lat:p1.lat*1 + (p2.lat - p1.lat) * eachPopulateStep / param2.steps});
        flights[p1.id].info.setPosition({lng:p1.lng*1 + (p2.lng - p1.lng) * eachPopulateStep / param2.steps, lat:p1.lat*1 + (p2.lat - p1.lat) * eachPopulateStep / param2.steps});
        ...
        $('.button').css('left',(param2.countTime/param2.fullTime)*100 + '%');
        $('.read').width((param2.countTime/param2.fullTime)*100 + '%');
        rotate(rollbackflight.div.id,{lng:p1.lng*1 + (p2.lng - p1.lng) * (eachPopulateStep - 1) / param2.steps, lat:p1.lat*1 + (p2.lat - p1.lat) * (eachPopulateStep - 1) / param2.steps},{lng:p1.lng*1 + (p2.lng - p1.lng) * eachPopulateStep / param2.steps, lat:p1.lat*1 + (p2.lat - p1.lat) * eachPopulateStep / param2.steps});
        eachPopulateStep++;
