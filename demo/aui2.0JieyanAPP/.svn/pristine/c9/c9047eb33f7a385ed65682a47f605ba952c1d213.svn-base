...
function codeConfirm(dialog) {
    ...
    $api.css($api.byId('titlePackage'), 'display:block');
    ...
    if (dialog == "aui-dialog-flight") {
        // $api.css($api.byId('titleFlight'), 'display:block');
        api.confirm({
            title: '温馨提示',
            msg: '您确定要创建订单吗？',
            button: ['确定', '取消']
...
function setFlightData(data) {
    ...
    if (user) {
        ...
        // alert('after curData is ' + JSON.stringify(curData));
        api.ajax({
...
function loadInitdata(data) {
    ...
    for (var i = 0; i < length; i++) {
        ...
        var html = '<li class="aui-list-view-cell aui-img" id="' + data[i].id + '"  tapmode onclick="openDetailWin(this)"><div class="aui-swipe-handle aui-arrow-right"><div class="aui-img-body order-item"><h2 class="item-title"><span>运单编号：</span><span id=\"' + data[i].boxid + '">S840320160627</span><span class="comment" id="comment' + data[i].id + '"></span></h2><span class="path-start"><i class="aui-iconfont aui-icon-locationfill cr1"></i><span id="sPoint">' + (JSON.parse(data[i].route)).airport[0].name + '</span></span><span class="path-end"><i class="aui-iconfont aui-icon-locationfill cr2"></i><span id="ePoint">' + (JSON.parse(data[i].route)).airport[1].name + '</span></span><div class="deltips"><span></span></div></div></div></li>';
        ...
        if (data[i].state == 0) {
            ...
            $api.text($api.byId("comment" + data[i].id), "已到达");
...
function initDetailData(data) {
    ...
    if (state == "0") {
        ...
        $api.text($api.byId('orderStatus'), "未起飞");
    } else if (state == "1") {
        $api.css($api.byId('orderCancelItem'), 'display:block');
        $api.css($api.byId('t2'), 'display:none');
        $api.css($api.byId('t4'), 'display:none');
    } else if (state == "2") {
        ...
        $api.css($api.byId('t3'), 'display:block');
    } else if (state == "4") {
        $api.text($api.byId('orderStatus'), "已到达");
    } else if (state == "5") {
        $api.text($api.byId('orderStatus'), "返航中");
    } else if (state == "7") {
        $api.text($api.byId('orderStatus'), "返航完成");
    } else {
        // alert(2);
        return;
    }
